...
function SideBar() {
    ...
    useEffect(() => {
        ...
        async function getTop(){
            const request = await axios.get("https://api.jikan.moe/v3/top/anime/1/bypopularity")
        ...
        async function getSeasonal(){
            const request = await axios.get("https://api.jikan.moe/v3/season/2022/summer")
            const temp = request.data.anime.slice(0,11);
            setSeasonal(temp);
        }
        getTop();
  ...
  return (
    <div className="sidebar">
        <div className="sideContainer1 sideContainer">
            ...
            {top.map(anime =>( 
                <a href={`https://animixplay.to/anime/${anime.mal_id}/${anime.title}`} target="_blank"><div key={anime.mal_id} className='sideanime'>
                    <img src={anime.image_url} alt='not available'></img>
                    <div>
                    <p>{anime.title.length > 60 ? `${anime.title.slice(0,60)}....` : anime.title }</p>
        ...
        <div className="sideContainer3 sideContainer">
            ...
            {upcoming.map(anime =>(
                <a href={`https://animixplay.to/anime/${anime.mal_id}/${anime.title}`} target="_blank"><div key={anime.mal_id} className='sideanime'>
                    ...
                    <p>{anime.title.length > 60 ? `${anime.title.slice(0,60)}....` : anime.title }</p>
                    <div>
                    <p className='airing' style={{color:'grey'}}><span style={{color:"white"}}>airing - </span>{anime.airing_start ? anime.airing_start.slice(0,10) : 'TBA'}</p>
                    </div>
                    </div>
                </div></a>
            ))}
